...
10. Création d’un lanceur `.exe` natif Windows
...
- **proche d’une vraie application Windows** (icône, scripts de lancement, etc.).
...
* De **transmettre des informations dynamiques** à Python au démarrage via des variables d'environnement (comme le rôle du processus — maître ou esclave — et l'état de la console).
...
1. Préparer l'icône de l'application *(Optionnel)*.
...
### 10.1 — Préparer l’icône du .exe *(Optionnel)*

Si vous souhaitez ajouter une icône personnalisée à votre exécutable, placez votre fichier dans :
...
[assembly: AssemblyCopyright("Copyright © 2026 Mon Éditeur.")]         // -> Droit d'auteur : Indiquer la mention de copyright
[assembly: AssemblyTrademark("Mon Éditeur")]                           // -> Marques légales : Indiquer la marque ou l'éditeur
[assembly: AssemblyCulture("")]                                        // -> Culture : Règle stricte -> DOIT RESTER VIDE ("")
[assembly: NeutralResourcesLanguage("fr-FR")]                          // -> Langue neutre : Indiquer le code langue (ex: fr-FR)
...
[assembly: AssemblyInformationalVersion("Version 1.0.0.0")]            // -> Version du produit : Indiquer la version publique visible

// Désactive les warnings de code mort/inaccessible pour les constantes de configuration
#pragma warning disable 0162, 0414, 0429

class Program {
    // Paramètres de configuration du lanceur
    private const bool SHOW_CONSOLE = true;                                    // Affichage console : true pour afficher le terminal, false pour masquer
    private const bool ENABLE_SINGLE_INSTANCE = false;                         // Sécurité instance unique : true pour empêcher les doubles lancements, false pour autoriser plusieurs instances (quoi qu'il arrive les instances multiprocessing seront autorisées)
    private static readonly string CONSOLE_TITLE = "Mon Application Terminal"; // Titre de la fenêtre de console si active
    ...
    private static readonly string SCRIPT_PATH = @"src\main.py";               // Script principal : Chemin relatif vers le fichier Python à lancer
    // FIN CONFIGURATION DES PARAMÈTRES

    // NOM UNIQUE DU MUTEX (Généré dynamiquement à partir de l'AppUserModelID)
    private static readonly string MUTEX_NAME = "Global\\" + APP_USER_MODEL_ID + "_Mutex"; // Nom unique du Mutex système global : utilise l'ID de l'application pour garantir un verrou matériel propre à ce programme
    ...
    private static void LogDebugInfo(int pid, string suffix, string status, string[] args, string logDir) {
        // Journal de debug pour analyser les lancements
        try {
    ...
    private static void CleanupOldMasterLogs(string logDir) {
        ...
        foreach (var file in datedFiles) {
            // Si on a déjà dépassé les 3 maîtres, tout le reste est trop ancien : on supprime
            if (masterLogCount >= 3) {
                try { file.Delete(); } catch { } // Ignore l'erreur si le fichier est ouvert (ex: Bloc-notes)
            }
            // Sinon, on compte ce fichier s'il s'agit du log du maître ("-LM.log")
    ...
    static void Main(string[] args) {
        // ====================================================================
        // ÉTAPE 1 : VÉRIFICATION IMMÉDIATE DE L'INSTANCE UNIQUE (EN TOUT PREMIER)
        // ====================================================================
        ...
        // Cas 2 : C'est un lancement manuel (l'utilisateur lance l'application)
        else {
        ...
        try {
            ...
            Environment.SetEnvironmentVariable("APP_IS_MASTER", isNewInstance ? (ENABLE_SINGLE_INSTANCE ? "1" : "2") : "0"); // 2 : Maître avec instances multiples (doit ouvrir un canal) | 1 : Maître en mode instance unique strict (verrouillé) | 0 : Instance secondaire
...
Voici une version optimisée et épurée. Le code est auto-explicatif : il traduit les valeurs techniques en labels lisibles et affiche tout directement dans votre interface Tkinter.
...
1. **Ce que l'exécutable transmet** (via les variables d'environnement et les arguments) : le rôle du processus, l'état de la console et les paramètres de ligne de commande.
2. **Ce que l'on applique de notre côté** dans le code Python : le regroupement des fenêtres dans la barre des tâches (qui utilise l'identifiant transmis) et l'application de l'icône de l'application (qui repose sur un fichier `.ico` totalement indépendant du `.exe`).

Voici un exemple complet de script Python (`main.py`) intégrant ces deux aspects, avec des valeurs par défaut pour que le code fonctionne aussi bien à l'intérieur du `.exe` qu'en phase de développement (par exemple sous VS Code).
...
# 1. RÉCUPÉRATION DES INFORMATIONS TRANSMISES PAR L'EXÉCUTABLE (.EXE)
# En cas de lancement manuel (VS Code), les valeurs par défaut prennent le relais.
app_role = os.environ.get("APP_IS_MASTER", "2")       # '0'=Esclave, '1'=Maître unique, '2'=Maître multi
app_id = os.environ.get("APP_USER_MODEL_ID", "MonEditeur.App.1.0")
has_console = os.environ.get("APP_HAS_CONSOLE", "1") == "1"
args = sys.argv[1:]  # Récupère les arguments de la ligne de commande

def setup_app(root):
    # Applique l'ID Windows pour fusionner les fenêtres dans la barre des tâches
    ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(app_id)
    
    # Force Windows à gérer correctement la haute résolution (DPI) pour éviter les interfaces floues
    try:
        ctypes.windll.shcore.SetProcessDpiAwareness(1)
    except:
        pass

    # Applique l'icône (chemin relatif simple basé sur le dossier du .exe)
    icon_path = os.path.join("assets", "app.ico")
    if os.path.exists(icon_path):
        root.iconbitmap(icon_path)

def main():
    # Si processus esclave (Multiprocessing), on quitte immédiatement
    if app_role == "0": return

    root = tk.Tk()
    root.title("Mon Application")
    setup_app(root)

    # 2. TRADUCTION DES INFORMATIONS POUR L'INTERFACE
    role_label = "Maître (Canal multi-esclaves)" if app_role == "2" else "Maître (Instance unique)"
    console_label = "Activé (Console visible)" if has_console else "Désactivé (Mode pur GUI)"

    # 3. AFFICHAGE DES INFORMATIONS DANS L'INTERFACE
    tk.Label(root, text="Informations de lancement", font=("Arial", 10, "bold")).pack(pady=10)
    tk.Label(root, text=f"Rôle : {role_label}").pack()
    tk.Label(root, text=f"Console : {console_label}").pack()
    tk.Label(root, text=f"ID Système : {app_id}").pack()
    
    if args:
        tk.Label(root, text=f"Arguments reçus : {' '.join(args)}", fg="blue").pack(pady=10)
